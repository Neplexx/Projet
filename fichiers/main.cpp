...
struct AvionVisuel {
    ...
    Coord position;
    Coord positionPrecedente;  // AJOUT�
    float tempsDepuisMaj;      // AJOUT�
    int altitude;
    int vitesse;
    int carburant;
    string villeDepart;
    ...
    AvionVisuel() : nom(""), id(0), selectionne(false), couleurBase(Color::White),
        position(Coord(0, 0)), positionPrecedente(Coord(0, 0)), tempsDepuisMaj(0.f),  // MODIFI�
...
Texture loadAvionTexture(const string& path) {
    ...
    if (!avionTexture.loadFromFile(path)) {
        ...
        if (!avionTexture.loadFromImage(img)) {
            cerr << "Erreur critique : Impossible de cr�er la texture par d�faut." << endl;
        }
    }
    return avionTexture;
}
...
int main() {
    ...
    // Initialisation syst�me de threads
    ...
    // Cr�er quelques avions initiaux
    for (int i = 0; i < 3; ++i) {
        ...
        cout << "Avion initial cr��: " << codeAvion
    ...
    while (app.isOpen()) {
        ...
        // G�n�ration automatique d'avions
        if (spawnClock.getElapsedTime().asSeconds() > tempsEntreAvions) {
            ...
            cout << "Nouvel avion cr��: " << codeAvion
        ...
        // R�cup�rer les positions des avions depuis les threads
        {
            ...
            for (const auto& entry : simulation.shared_data->avions_positions) {
                ...
                // MODIFI�: Interpolation des positions
        ...
        // Incr�menter le temps d'interpolation pour tous les avions
        for (auto& pair : avionsVisuels) {
            pair.second.tempsDepuisMaj += deltaTime;
        }

        // R�cup�rer infos d�taill�es
        ...
        // MODIFI�: Dessin avec interpolation
        for (const auto& pair : avionsVisuels) {
            if (pair.second.sprite.has_value()) {
